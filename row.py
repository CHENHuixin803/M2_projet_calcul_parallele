...
if __name__ == '__main__':
    ...
    if not benchmark_mode:
        ...
        all_local_nys = globCom.gather(grid.ny_loc, root=0)
        if rank == 0:
            recvcounts = tuple([ny * global_dim[1] for ny in all_local_nys])
        ...
        else: recvcounts = displacements = global_cells_flat = None
        ...
        grid.cells[1 : grid.ny_loc + 1, :] = global_initial_cells[grid.y_start : grid.y_start + grid.ny_loc, :]

        all_local_nys = globCom.gather(grid.ny_loc, root=0)
        if rank == 0:
            recvcounts = tuple([n_y * global_dim[1] for n_y in all_local_nys])
            displacements = tuple([sum(recvcounts[:i]) for i in range(nbp)])
            global_cells_flat = np.empty(global_dim[0] * global_dim[1], dtype=np.uint8)
        ...
        if dump_grid:
            local_real_cells = grid.cells[1 : grid.ny_loc + 1, :].flatten()
            globCom.Gatherv(local_real_cells, [global_cells_flat, recvcounts, displacements, MPI.BYTE], root=0)
            if rank == 0: global_cells_2d = global_cells_flat.reshape(global_dim)
